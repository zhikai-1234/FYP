...
def callBack(data, args):
    ...
    try:
        if data.header.frame_id == '' or data.header.frame_id == 'world':
            data.header.frame_id = args[1]
...
# clearing old frontiers
        ...
        while z < len(centroids):
            ...
            for i in range(0, n_robots):
                ...
                try:
                    target_frame = namespace+str(i+namespace_init_count)+'/map'
                    transformedPoint = tfLisn.transformPoint(target_frame, temppoint)
                    x = array([transformedPoint.point.x, transformedPoint.point.y])
                    cond = (gridValue(globalmaps[i], x) > threshold) or cond
                except Exception as e:
                    pass
            if (cond or (informationGain(mapData, [centroids[z][0], centroids[z][1]], info_radius*0.5)) < 0.0):
...
# publishing
        ...
        pp = []
        frontiers_snap = frontiers[:]
        for q in range(0, len(frontiers_snap)):
            if q < len(frontiers_snap) and len(frontiers_snap[q]) >= 2:
                p.x = frontiers_snap[q][0]
                p.y = frontiers_snap[q][1]
                pp.append(copy(p))
        points.points = pp
        pp = []
        centroids_snap = centroids[:]
        for q in range(0, len(centroids_snap)):
            if q < len(centroids_snap) and len(centroids_snap[q]) >= 2:
                p.x = centroids_snap[q][0]
                p.y = centroids_snap[q][1]
                pp.append(copy(p))
